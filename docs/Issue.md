...
## #50 RAG 将 Top-K 候选全部展示为实际引用

- **日期**：2026-09-04
- **模块**：`RagService`、`ChatClient`、AI 问答引用展示
- **现象**：问题“吃了什么”只依据一篇日记回答并仅标注 `[#1]`，接口却返回 5 条 `citations`，页面因而把另外 4 篇无关文档也显示为引用来源。新增第 6 篇文档后仍固定返回前 5 篇，确认展示数量来自默认 `topK=5`，不是全部文档查询。
- **根因**：RAG 相似度查询只排序和限制数量，没有相关性阈值；`ChatClient` 在模型回答前就把全部候选映射为 `citations`，返回时未根据回答实际使用的 `[#n]` 过滤。
- **生产校准证据**：同一问题前 6 个候选只输出分数、不读取或输出正文，分布为 `0.4408 / 0.3532 / 0.3486 / 0.3340 / 0.2412 / 0.2156`。只使用 `0.30` 绝对阈值仍会错误保留 4 篇。
- **修复**：候选同时满足绝对相似度 `>= 0.30` 和相对最佳结果 `>= 85%` 才进入模型上下文；回答完成后仅保留实际出现的合法 `[#n]`，按首次出现顺序重新编号并同步裁剪 `citations`，移除模型生成的越界编号。
- **验证**：新增 6 个单元测试覆盖生产分数分布、多篇强相关、低于绝对阈值、部分引用重编号、越界引用和无引用回答；Server 单元测试 `64 passed / 1 skipped / 0 failed`，Server typecheck、仓库级 typecheck 与本次 4 个文件的定向 ESLint 通过。仓库级 lint 仍受既有错误阻断，与本项无关。
